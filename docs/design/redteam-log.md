...
## Round 2 (2026-08, 攻撃者 3 名: 修正の反証検証 / 新角度 / 過剰設計批判)

Round 1 の修正案そのものを攻撃した結果、修正の多くが「書かれた形のままでは目的を
達成しない」ことが判明した。詳細は各エージェント出力(要旨):

- **R2-1**: `registry:build` が非決定的(index に `updatedAt: now`、shallow clone で
  `gitDates` が壊れる)。S1 の `git diff --exit-code` と外部モニタが原理的に動かない。
  → S4 にビルド決定化を追加。
- **R2-3 / D1**: `id: meta.id || dirname` で別ディレクトリから既存 id を名乗れる →
  Tier 0 自動マージ経由でアイテム乗っ取り(URL/ハッシュ奪取)。→ S11 に `meta.id`=
  dirname 強制を追加(CI 骨組みで実装)。
- **R2-6 / B系**: `vault.fetch` の第2引数(`connectionRef`/`path`)と一級関数経由は
  静的導出できない。第一引数リテラル限定を素通りする。→ S10 に「静的導出の天井」を
  明記し、導出不能な引数は赤フラグ → 人手。
- **A1(S7 preauth)**: MiAuth の confused-deputy は preauth Cookie でも閉じない
  (start↔承認者の束縛が MiAuth に存在しない)。→ **Round 3 でログインごと廃止したため moot**。
- **過剰設計批判(最重要)**: 外部モニタ・index 署名・HMAC・7日異議申立・device token は
  単独運営の脅威モデルに対し過剰。835 行の仕様と Phase 0 の重量が、最も深刻な脅威 T10
  (運営消滅)の発生確率を自ら上げている。MVSS(最小出荷可能安全サブセット)= R1-C 修正 +
  S7 を正しく書く + revoke.json + 正直な PRIVACY + ログイン任意化。
  → **Round 3 のアーキテクチャ決定に反映(下記)**。

---

## Round 3 (2026-08, アーキテクチャ決定 — 運営者による方針確定)

Round 2 の過剰設計批判と運営者の判断が一致し、設計を単純化。これにより Round 1/2 の
反証の一部が**アーキテクチャごと消滅(moot)**した。何を消したか、なぜ消えたかを記録する
(原則文書が「削除理由を残せ」と要求するため)。

**決定した構成**:
1. 配布は GitHub PR のみ(Misskey ログインなし)。作者同一性 = GitHub アカウント。
2. 審査は GitHub Actions(機械チェック)+ 高権限のみ人手。
3. 利用状況はストアが持たず NoteDeck クライアントがローカル所有(ユーザー主権)。
4. ☆評価・DL 数は出さない(design.md 118-120 維持 + Sybil で機能しないため)。客観
   signal のみ。
5. ログイン・開発者の Misskey↔GitHub 紐付けも当面なし(必要なら将来 rel=me 相互リンクで)。

**moot になった反証(受け皿がアーキテクチャごと消えた)**:

| 反証 | なぜ moot か |
|---|---|
| R1-1〜13, R2-A1〜A6(MiAuth 認証系すべて) | **ログインを持たない**。認証・セッション・トークン・confused-deputy・CSRF・SSRF の攻撃面が存在しない |
| R1-R, R2-14(ライブラリ privacy / GDPR / HMAC) | **サーバー側ライブラリを持たない**。ユーザーデータが存在しないので漏れようがない (P6) |
| R1-S, R2-13(ログインフィッシング / device token) | **ログイン導線が無い**ので偽ストアの MiAuth 誘導・device token 詐取が成立しない |
| R1-9, R2-6(rating の Sybil) | **評価機能を作らない**ので Sybil で汚す対象が無い |
| R2-5, R1-T の署名部(index 署名の鍵管理) | **署名を当面見送り**、長期ドメイン登録 + staleness 警告で受ける |
| 外部モニタの常設・別 Cloudflare アカウント | **過剰**と判断。CI + 数回/年の手動突合に降格 |

**生き残った反証(アーキテクチャ非依存 = レジストリ完全性そのもの。CI が守る対象)**:

| 反証 | 反映先 |
|---|---|
| R1-C(生成物三重コミット = レビュー対象≠配布物) | S1/S11、Phase 1 |
| R2-3/D1(`meta.id` 乗っ取り) | S11、CI 骨組みで実装 |
| R2-1/R2-2(ビルド非決定性・日付自己申告) | S4、Phase 1 |
| R1-A/H/I, R2-6(全ホスト関数導出・エイリアス・第2引数の天井) | S10、Phase 2 |
| R1-L/R2-17(homoglyph/bidi を name にも・ZWJ 例外・表示分離) | S9、CI 骨組みで一部 |
| R1-G/K, R2-10/11/12(skills injection・クロス Tier・frontmatter パーサ) | S13 |
| R1-E/F(queries=実行コード・themes コントラスト) | S2 |
| R2-19/20(SPA fallback で 404→200・`_headers` allowlist 漏れ・SVG XSS) | S5/S12 |

**総括**: 「攻撃面を防御する」から「攻撃面の存在を無くす」へ。認証とサーバー側個人情報を
持たないことで、赤チーム 2 ラウンドの反証の過半が実装不要になった。残るのはレジストリ
自体の完全性(可読ソース = 配布バイト = ハッシュの一致)であり、これは CI の仕事。
