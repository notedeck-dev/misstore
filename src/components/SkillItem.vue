    ...
    <div class="vsx-body">
      <div class="vsx-icon-plain" style="color: var(--accent-text)">
        <span
          ...
          role="img"
          :aria-label="itemText(skill).name"
        ></span>
      ...
      <div class="vsx-details">
        <div class="vsx-name">{{ itemText(skill).name }}</div>
        ...
        <p class="vsx-desc">{{ itemText(skill).description }}</p>
      </div>
    </div>
    <div class="vsx-footer">
      <div class="vsx-actions">
        <CopyButton :source-url="skill.sourceUrl" :id="skill.id" />
      </div>
    </div>
  </router-link>
</template>
